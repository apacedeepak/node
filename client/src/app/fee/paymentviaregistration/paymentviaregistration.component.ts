...
@Component({
  selector: 'app-paymentviaregistration',
  templateUrl: './paymentviaregistration.component.html',
  styleUrls: ['./paymentviaregistration.component.css']
})
export class PaymentviaregistrationComponent implements OnInit {
  id: number;
  private sub: any;
  searchParam: any;
  studentDetails: any = [];
  ...
  studentPersonalDetails(student_id) {
    ...
    this.searchParam = { "type": "student", "user_id": student_id,"status":"Unpaid" };
    this.http.post(url, this.searchParam).subscribe(data => {
      ...
      if (result.response_status.status == "200") {
        ...
        var url2 = this.myservice.constant.apiURL + "sections/sectionbyid";
        this.http.post(url2, sections).subscribe(datas => {
          const results: any = datas;
          this.classId=results.response.section_class.classId;
          this.boardId=results.response.board.boardId;
        });
        //console.log(">>>>>>>>>")
...
if(value.transaction_date){
  ...
  req.transaction_month = value.transaction_date.year.toString() + '-' + transaction_month + '-' + value.transaction_date.day.toString();
...
this.http.post(url, req).subscribe(data => {
  ...
  if (result.response_status.status == "200") {
    ...
    this.http.post(this.myservice.constant.apiURL+ "registration/updatestatus",update ).subscribe(data => {
      const datas:any=data
      if(datas.response_status.status=="200"){
        alert(" Student Registered and Payment has been done successfully...");
