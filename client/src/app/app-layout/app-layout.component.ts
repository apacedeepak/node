...
export class AppLayoutComponent implements OnInit {
  ...
  private https: HttpClient,private http: Http, private fb: FormBuilder,private translate: TranslateService) {
    ...
    translate.setDefaultLang(this.transLang);
      ...
      this.myhrUrl = window.localStorage.getItem('myhrLoginUrl');
  }
   ...
   // alert(this.global.school_id);
    this.global.domain = this.myService.constant.domainName;
     ...
     this.global.userTypeData = "("+window.localStorage.getItem('user_type')+")";
      ...
      this.global.role_name = window.localStorage.getItem('role_name');
      this.global.username = window.localStorage.getItem('username');
      this.global.name = window.localStorage.getItem('name');
...
//      });
    ...
    //this.https.get(this.myService.constant.apiURL + 'layoutmenulist/layoutmenu?time='+n).subscribe(details => {
      ...
      let roleName = this.global.role_name.toLowerCase().replace(/ +/g, "");
    this.https.get(this.myService.constant.apiURL + 'leftmenu/leftmenu?product_type='+roleName+'&time='+n).subscribe(details => {
...
if(menu.sub_menu.find(subMenu=>subMenu.path==this.navUrl)){
  menu.navUrl="show"
   ...
   let params = {"token": this.global.token, "school_id":window.localStorage.getItem('school_id'),"allstatus":1};
    if(this.global.userType.toLowerCase() == 'student' || this.global.userType.toLowerCase() == 'parent' || this.global.userType.toLowerCase() == 'teacher'){     
      params.allstatus=0;
    }
      ...
      if(this.global.userType.toLowerCase() == 'parent'){ 
          ...
          this.https.post(this.myService.constant.apiURL + 'users/userdetail', params).subscribe(details => {
            ...
            this.https.post(this.myService.constant.apiURL + 'users/userdetail', param).subscribe(details => {
                ...
                //console.log(this.multischoolArr);
                ...
                if(this.multischoolArr!=undefined && this.multischoolArr.length>1 && currentlocation.indexOf("dashboard/main") != -1)
                {
                  ...
                  this.multi_select_val = this.multischoolArr[0].id;
...
//  });
  ...
  logout(){
    ...
    if (r == true) {
      ...
      {
        window.localStorage.setItem('messages', "You have successfully logged out");
