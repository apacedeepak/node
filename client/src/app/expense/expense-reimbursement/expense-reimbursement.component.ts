...
export class ExpenseReimbursementComponent implements OnInit {
  expenseReimbursementForm: FormGroup;
  searchReimbursementForm: FormGroup;
  ...
  public expense_reimbursement_title: any = "Reimbursement";
  public params: any = {};
  ...
  public expense_for_show: boolean = false;  
  ...
  ngOnInit() {
    ...
    this.expenseReimbursementForm = new FormGroup({
      id: new FormControl(""),
  ...
  getAutoLoadAfterEvent() {
   ...
   // this.getExpenseMasterList(2);
    this.onSearchSubmit(this.Searchparams);
    this.getUserCenterList();
    this.searchReimbursementForm = new FormGroup({
  ...
  getCategoryMasterList() {
    ...
    const url =
      ...
      "expense_category_masters/getcategoryreimbursment?userId=" + this.userId;
    this.http.get(url).subscribe(response => {
      const data: any = response;
      this.globalObj.categoryMasterList = data.response;
      // this.expenseReimbursementForm.patchValue({
      //   expense_category: 2
      // });
    });
  }
  ...
  onSubmit(formValue) {
    ...
    formData.append("expense_for", formValue.expense_for);    
    ...
    formData.append("expense_type", '2');
    formData.append("id", formValue.id);   
    for (var i in expense_doc) {
       formData.append(i, expense_doc[i].file);
    } 
    if(parseInt(this.globalObj.totalamount) <= 0){
      this.errorMessage.message = "Total amount can not be less than equal to 0";
    ...
    this.http
      ...
      .subscribe(data => {
        ...
        if (details.status == "200") {
          this.successMessage.message = "Reimbursement filled sucessfully";
        ...
        this.expense_reimbursement_title = "Reimbursement List";
        this.getFromSet();
        this.getAutoLoadAfterEvent();
      });
    }
  ...
  editExpenseRequest(expenseRequestId) {
    if (expenseRequestId) {
      this.show_add_item_master = true;
      this.show_item_master_list = false;
      this.expense_reimbursement_title = "Edit Reimbursement";
      this.editOrUpadte = 1;
      this.http
        .get(
          this.myService.constant.apiURL +
        ...
        .subscribe(detail => {
          ...
          this.globalObj.totalamount =parseInt(this.expenseRequestData.amount) + parseInt(this.expenseRequestData.gst_amount);      
          this.expenseReimbursementForm.patchValue({
