...
export class VendorPaymentComponent implements OnInit {
  ...
  public vendor_payment_title: any = "Vender Payment";
  public params: any = {};
  ...
  ngOnInit() {
    ...
    this.fileExpenseForm = new FormGroup({
      ...
      expense_for: new FormControl("2", Validators.required),
      ...
      expense_amount: new FormControl("0",Validators.required),
      gst_amount: new FormControl("0",Validators.required),
  ...
  onSubmit(formValue) {
    ...
    formData.append("expense_type", '3');
    formData.append("id", formValue.id);   
    for (var i in expense_doc) {
       formData.append(i, expense_doc[i].file);
    } 
    if(parseInt(formValue.vendor_total_amount) < 10001){
      this.errorMessage.message = "Total amount can not be less than 10001";
    ...
    this.http
      ...
      .subscribe(data => {
        ...
        if (details.status == "200") {
          this.successMessage.message = "Vendor payment filled sucessfully";
        ...
        this.vendor_payment_title = "Vendor Payment List";
        this.getFromSet();
        this.getAutoLoadAfterEvent();
      });
    }
  ...
  editExpenseRequest(expenseRequestId) {
    if (expenseRequestId) {
      this.show_add_item_master = true;
      this.show_item_master_list = false;
      this.vendor_payment_title = "Edit Vendor Payment";
      this.editOrUpadte = 1;
      this.http
        .get(
          this.myService.constant.apiURL +
        ...
        .subscribe(detail => {
          ...
          this.fileExpenseForm.patchValue({
            ...
            total_amount:this.expenseRequestData.total_amount
          });
